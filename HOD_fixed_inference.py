...
class Likelihood:
    ...
    def run_chain_test(
        ...
        np.random.seed(4200)
        initial_step   = cosmo_params + 1e-3 * np.random.normal(0, 1, size=(self.nwalkers, self.nparams))
        ...
        for ii, (pos, prob, state) in enumerate(sampler.sample(initial_step, iterations=5, progress=True, skip_initial_state_check=True)):
            print(f"{ii=}")
            # print(f"{pos=}")
            continue
...
L4.continue_chain("fixed_HOD_DE_4w_5e4.hdf5", check_convergence=False, max_new_iterations=int(2e5), moves=emcee.moves.DEMove())
